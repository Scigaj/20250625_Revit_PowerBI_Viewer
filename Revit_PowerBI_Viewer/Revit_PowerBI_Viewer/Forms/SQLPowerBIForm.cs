...
namespace Revit_PowerBI_Viewer.Forms
{
    public partial class SQLPowerBIForm : System.Windows.Forms.Form
    {
        ...
        private void btn_Export_Click(object sender, EventArgs e)
        {
            ...
            foreach (DataGridViewRow row in dg_Categories.Rows)
            {
                ...
                var cell = row.Cells["Categories"];
                if (cell == null || cell.Value == null) continue;

                string selectedCategory = cell.Value.ToString();
                    ...
                    if (value == selectedCategory)
                    {
                        ...
                        foreach (BuiltInCategory bic in builtInCatArray)
                        {
                            ...
                            if (cat == selectedCategory)
                            {
                                ...
                                bool doesExist = TableExists("Revit_API_SQL", category);
                                ...
                                foreach (var dictPair in map_cat_to_uid_to_param_values)
                                {
                                    if (dictPair.Value.Count > 0)
                                    {
                                        foreach (var innerPair in dictPair.Value)
                                        {
                                            ...
                                            //Check if table is created if so just save values.
                                            doesExist = TableExists("Revit_API_SQL", category);
        ...
        private void BindData()
        {
            ...
            dg_Categories.DataSource = dgCategories;
            ...
            dg_Categories.Columns.Insert(0, checkBoxColumn);

            //Change the AutoSize mode to fill the width of the DataGridView as it resizes
            dg_Categories.Columns[0].AutoSizeMode = DataGridViewAutoSizeColumnMode.AllCells;
            dg_Categories.Columns[1].AutoSizeMode = DataGridViewAutoSizeColumnMode.Fill;

            //dataGrid_Categories.BackgroundColor = dataGrid_Categories.DefaultCellStyle.BackColor;
            //Sort list in ascending order
            dg_Categories.Sort(dg_Categories.Columns["Categories"], ListSortDirection.Ascending);
        ...
        private void dg_Categories_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            foreach (DataGridViewRow row in dg_Categories.Rows)
            {
        ...
        private void txt_Search_TextChanged(object sender, EventArgs e)
        {
            ...
            dv.RowFilter = "Categories LIKE '" + txt_Search.Text + "%'";

            dg_Categories.DataSource = dv;
